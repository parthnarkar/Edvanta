...
try:
    ...
    print("✅ Connected to MongoDB for chatbot")
...
def load_chat_sessions():
    ...
    try:
        sessions = list(chat_sessions_col.find({identifier_field: identifier}).sort("lastActivity", -1))
        for s in sessions:
            fix_id(s)
    ...
    try:
        ...
        for session in sessions:
            if session:
                ...
                session["_id"] = ObjectId(session_id) if session_id else ObjectId()
...
def update_session_messages(session_id):
    ...
    try:
        ...
        print(f"❌ Error updating session messages: {e}")
...
def delete_chat_session(session_id):
    ...
    try:
        chat_sessions_col.delete_one({"_id": ObjectId(session_id), identifier_field: identifier})
...
def send_message():
    ...
    try:
        print(f"🤖 Processing doubt: {user_message}")
        
        # Get AI response with full conversation context
        ai_response = get_ai_response(user_message, context="", chat_history=chat_history)
...
Would you like me to elaborate on any specific aspect of this topic?"""
        ...
        if session_id:
            try:
                ...
                print(f"⚠️ Failed to update session: {session_error}")
        ...
        print("✅ Doubt solved successfully")
    ...
    if context:
        ...
        for line in lines:
            if line.startswith('Student:'):
                chat_history.append({"role": "user", "content": line.replace('Student:', '').strip()})
            elif line.startswith('Tutor:'):
                chat_history.append({"role": "assistant", "content": line.replace('Tutor:', '').strip()})
...
Would you like me to elaborate on any specific aspect of this topic?"""
        ...
        print(f"❌ Error in legacy ask endpoint: {e}")
        return jsonify({"error": f"Internal server error: {str(e)}"}), 500
