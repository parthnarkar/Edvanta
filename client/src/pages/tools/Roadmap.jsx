...
export function Roadmap() {
  ...
  const fetchUserRoadmaps = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backEndURL}/api/roadmap/user?user_email=${user.email}`);
  ...
  const generateCustomRoadmap = async () => {
    ...
    try {
      ...
      console.log("Custom roadmap generated:", parsedRoadmap);
  ...
  const viewRoadmapDetails = async (roadmap) => {
    ...
    try {
      ...
      const response = await fetch(`${backEndURL}/api/roadmap/${roadmap.id}?user_email=${user.email}`);
  ...
  const deleteRoadmap = async (roadmapId) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`${backEndURL}/api/roadmap/${roadmapId}?user_email=${user.email}`, {
        method: 'DELETE',
      });
      ...
      console.log("Roadmaps refetched after deletion");
    ...
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 lg:p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tell us about your goals and current skills to get a personalized learning path
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {!user ? (
            <div className="bg-yellow-50 border border-yellow-100 rounded-md p-4 text-center">
              <p className="text-sm text-yellow-800 mb-2">Please sign in to create roadmaps</p>
              <Button
                variant="outline"
                className="bg-white"
                onClick={() => {
            ...
            <div className="space-y-3">
              <div>
                <label htmlFor="goal" className="block text-sm font-medium mb-1">What's your learning goal?</label>
                <Input
                  id="goal"
              ...
              <div>
                <label htmlFor="background" className="block text-sm font-medium mb-1">What's your current background?</label>
                <Input
                  id="background"
              ...
              <div>
                <label htmlFor="duration" className="block text-sm font-medium mb-1">Target completion time (weeks)</label>
                <Input
                  id="duration"
                  type="number"
                  placeholder="Eg. 12"
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-900">Your Generated Roadmaps</h2>
        ...
        {!user ? (
          <div className="bg-yellow-50 border border-dashed border-yellow-200 rounded-lg p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">Please sign in to view your roadmaps</h3>
          ...
          <div className="bg-gray-50 border border-dashed border-gray-200 rounded-lg p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">Loading your roadmaps</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {savedRoadmaps
              ...
              .map((roadmap) => (
              <Card key={roadmap.id} className="hover:shadow-md transition-all duration-200">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base sm:text-lg">{roadmap.title}</CardTitle>
                  <CardDescription className="text-xs">
                    Created: {roadmap.dateCreated} • Duration: {calculateTotalDuration(roadmap.data.nodes)}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div>
                    <p className="text-xs text-gray-600 line-clamp-2">{roadmap.description}</p>
                    <div className="flex flex-wrap gap-1 mt-2">
                      ...
                      {roadmap.data.nodes && roadmap.data.nodes.length > 3 && (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      className="w-full text-xs sm:text-sm text-red-600 hover:bg-red-50 border-red-200"
                      onClick={(e) => {
                        e.stopPropagation();
          ...
          <div className="bg-gray-50 border border-dashed border-gray-200 rounded-lg p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">No roadmaps generated yet</h3>
            <p className="text-gray-500 mb-6 max-w-md mx-auto">
              Fill out the form above to generate your first personalized career roadmap.
            </p>
            <Button
              variant="outline" 
              onClick={() => {
          ...
          <div className="bg-white/95 backdrop-filter rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden border border-gray-100 flex flex-col">
            ...
            <div className="sticky top-0 bg-white/80 backdrop-blur-sm z-10 border-b p-4 rounded-t-xl">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2 sm:gap-3 max-w-[80%]">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-xl font-bold text-gray-900 line-clamp-1">{selectedRoadmap.title}</h2>
                    <p className="text-xs sm:text-sm text-gray-500">Created on {selectedRoadmap.dateCreated}</p>
                  </div>
                </div>
                <button 
            ...
            <div className="p-4 my-2 space-y-6 overflow-y-auto flex-1">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-3 sm:p-5 rounded-xl shadow-sm border border-blue-100">
                ...
                <p className="text-sm text-gray-700 mb-3 sm:mb-4 leading-relaxed break-words">{selectedRoadmap.description}</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                  <div className="bg-white/80 backdrop-blur-sm p-2 sm:p-3 rounded-lg flex items-center gap-2 sm:gap-3 shadow-sm">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm text-gray-500 truncate">Total Duration</p>
                      <p className="text-sm sm:text-base font-medium truncate">{calculateTotalDuration(selectedRoadmap.data.nodes)}</p>
                    </div>
                  </div>
                  <div className="bg-white/80 backdrop-blur-sm p-2 sm:p-3 rounded-lg flex items-center gap-2 sm:gap-3 shadow-sm">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm text-gray-500 truncate">Skills to Learn</p>
                      <p className="text-sm sm:text-base font-medium truncate">{selectedRoadmap.data.nodes?.length - 1 || 0}</p>
                    </div>
                  </div>
                  <div className="bg-white/80 backdrop-blur-sm p-2 sm:p-3 rounded-lg flex items-center gap-2 sm:gap-3 shadow-sm">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm text-gray-500 truncate">Target Completion</p>
                      <p className="text-sm sm:text-base font-medium truncate">{new Date(new Date(selectedRoadmap.dateCreated).getTime() + (selectedRoadmap.duration * 7 * 24 * 60 * 60 * 1000)).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {selectedRoadmap.data.nodes?.map((node, index) => (
                    <div key={node.id || index} className="relative">
                      ...
                      <div className="flex gap-3 sm:gap-5">
                        {/* Step Icon */}
                        <div className={`flex-shrink-0 w-9 h-9 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shadow-md z-[5] ${
                          index === 0 
                          ...
                          <Card className="bg-white/90 backdrop-blur-sm border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative z-[1] overflow-hidden">
                            <CardHeader className="pb-2 sm:pb-3 px-3 sm:px-6 pt-3 sm:pt-4">
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
                                <CardTitle className="text-sm sm:text-base md:text-lg text-gray-800 line-clamp-2 break-words">{node.title}</CardTitle>
                                {node.recommended_weeks && (
                                  <Badge variant="outline" className="text-xs whitespace-nowrap bg-blue-50 text-blue-600 border-blue-200 w-fit flex-shrink-0">
                                    {node.recommended_weeks} {node.recommended_weeks === 1 ? 'week' : 'weeks'}
                                  </Badge>
                                )}
                              </div>
                            </CardHeader>
                            <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-4">
                              <p className="text-xs sm:text-sm text-gray-600 leading-relaxed break-words">{node.description}</p>
                                ...
                                <div className="bg-gray-50/80 rounded-lg p-2 sm:p-3">
                                  ...
                                  <ul className="text-xs sm:text-sm text-gray-600 space-y-1.5 sm:space-y-2 pl-4 sm:pl-5">
                                    {node.resources.map((resource, i) => (
                                      <li key={i} className="list-disc marker:text-blue-400 break-words">
                                        <span className="text-gray-700 break-words">
                                          {typeof resource === 'string' ? resource : resource.name || resource.title || 'Resource'}
                                        </span>
                                      </li>
                                    ))}
                                  </ul>
              ...
              {selectedRoadmap.data.edges && selectedRoadmap.data.edges.length > 0 && (
                <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-3 sm:p-5 rounded-xl shadow-sm border border-indigo-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                    {selectedRoadmap.data.edges.map((edge, index) => {
                      const fromNode = selectedRoadmap.data.nodes.find(n => n.id === edge.from);
                      const toNode = selectedRoadmap.data.nodes.find(n => n.id === edge.to);
                      
                      if (!fromNode || !toNode) return null;
                      
                      return (
                        <div key={index} className="bg-white/80 backdrop-blur-sm p-2 sm:p-3 rounded-lg flex items-center gap-2 shadow-sm overflow-hidden">
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm overflow-hidden min-w-0 flex-1">
                            <span className="font-medium text-gray-800 line-clamp-1 min-w-0 flex-shrink truncate">{fromNode.title}</span>
                            <ArrowRight className="h-3 w-3 text-gray-400 flex-shrink-0 mx-1" />
                            <span className="font-medium text-gray-800 line-clamp-1 min-w-0 flex-shrink truncate">{toNode.title}</span>
                          </div>
                        </div>
                      );
                    })}
